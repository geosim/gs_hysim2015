...
/*+
	Determina il massimo numero (cioè composto da tutti 9) che può essere 
...
/*+
	Determina il minimo numero (cioè composto da tutti 9) che può essere
	...
	TRY
	{
		...
		if ((*count) > 0)
		{
         ...
         StatusBarProgressMeter.Init(*count);
			...
			// se l'entità è "nuova" (ovvero è stata inserita nella sessione corrente) oppure è nella lista delle
			// cancellate si controlla l'esistenza di tale entità in grafica.
            ...
            StatusBarProgressMeter.Set(++i);
				...
				// controlla se l'entità è nella lista delle cancellate
			...
			while (gsc_isEOF(pRs) == GS_BAD)
         {
            StatusBarProgressMeter.Set(++i);
				...
				if (in_graphic)
				{
					...
					{
						// verifica se il parametro "nodo_iniz" è un REGOLATORE
						if (sub2.nodo_iniz != 0)
						...
						// verifica se il parametro "nodo_final" è un REGOLATORE
					...
					// della SIMULAZIONE, verifica se il parametro "nodo_iniz" è un PRV.
					if (prvN>0)
					{
					...
					// parametrico di SELECT per le VALVOLE della SIMULAZIONE,
  					if (valvoleN>0)
					{
						// verifica se il parametro "nodo_final" è una VALVOLA.
						...
						// verifica se il parametro "nodo_iniz" è una VALVOLA.
...
// roby per regolatori di taglio
         ...
         StatusBarProgressMeter.End(gsc_msg(1090)); // "Terminato."
			...
			if (idMsg.GetLength() > 0)
			{
				strMsg = "\nI regolatori ";
				strMsg += idMsg;
				strMsg += " presentano più linee di uscita, la simulazione viene interrotta in quanto i risultati non sarebbero attendibili. Modificare affinchè da ciascun regolatore esca una singola linea.";
				acutPrintf(strMsg);
				AfxThrowUserException( );
			}
		}
            ...
            StatusBarProgressMeter.Set(++i);
               ...
               str = _T("0,");
                  ...
                  // Leggo in ANSI
						...
						{ 
							hy_errmsg(HY_STRING224); // "La curva caratteristica di una pompa della rete non è definita nel file <pompe.ini>."
							AfxThrowUserException();
						}
...
/*+
   ...
   della complessità dei file finali, che risultano infatti una 
...
int CHydroMap::OutFromGEOsim()
{
	...
	// SERBATOI/REGOLATORI
		...
		lastN = pSub->GetNewEntCode();			// relativamente alle entità nodali
		...
		// VALVOLE (nel caso di simulazione ACQUA il codice identificativo della classe <VALVOLE> è 5)
		...
		else // nel caso di simulazione GAS
		{
		   ...
		   // VALVOLE (nel caso di simulazione GAS il codice identificativo della classe <VALVOLE> è 4)
			if ((pSub = gsc_find_class(prj, cls, SUB4)) == NULL)
				AfxThrowUserException();
			campi = pSub->ptr_info()->key_attrib.get_name();
			campi += _T(", QUOTA, CHIUSA");
...
/*+
	...
	- "gs_id"	identificatore dell'oggetto da ricercare
	Ottenuto il cursore sui dati aggiorna la riga con:
	- "value1"	nuovo valore da assegnare alla quantità uscente
							...
							double value1, double value2, long gs_id, int *found)
{
   ...
   int            IsRecordsetCloseable = GS_GOOD;
	...
	TRY 
	{
		...
		if (q_uscente > max)			// q_uscente è il valore impostato dalla funzione gsc_in_sub3(...),
			q_uscente = 0;				// in particolare è arrotondato al valore superiore.
...
/*+
	Aggiorna a video le entità determinate eseguendo una SELECT sul campo 
...
/*+
	Aggiorna a video le entità determinate eseguendo una SELECT sui campi 
...
int CHydroMap::InToGEOsim()
{
   ...
   StatusBarProgressMeter.Init(4); // 3 fasi
	...
	TRY
	{
      ...
      msg2.LoadString(IDS_STRING277); // "entità puntuali"
      C_STATUSLINE_MESSAGE StatusLineMsg(_T("%s %s"), msg, msg2);
      StatusLineMsg.Init(gsc_msg(310), MEDIUM_STEP); // ogni 100 "%ld entità GEOsim elaborate."
		...
		{
			StatusLineMsg.Set(++count); // "%ld entità GEOsim elaborate."
      ...
      StatusLineMsg.End(gsc_msg(310), count); // "%ld entità GEOsim elaborate."
		
      StatusBarProgressMeter.Set(3); // terza fase

		msg.LoadString(IDS_STRING279); // "Aggiornamento"
      msg2.LoadString(IDS_STRING280); // "entità lineari"
      C_STATUSLINE_MESSAGE StatusLineMsg2(_T("%s %s"), msg, msg2);
      StatusLineMsg2.Init(gsc_msg(310), MEDIUM_STEP); // ogni 100 "%ld entità GEOsim elaborate."
                              ...
                              &diametro) != EOF)
		{
			...
			StatusLineMsg2.Set(count); // "%ld entità GEOsim elaborate."
      ...
      StatusLineMsg2.End(gsc_msg(310), count); // "%ld entità GEOsim elaborate."

		// Alcune entità potrebbero non essere coinvolte nell'ultima simulazione eseguita,
		// (esempio esistenza valvola chiusa),
		// è pertanto necessario aggiornare tali entità con valori nulli.
         ...
         StatusBarProgressMeter.Init(*count);
            ...
            StatusBarProgressMeter.Set(++i);
				...
				if (in_graphic)
				{
					...
					// la seguente funzione, utilizzando lo statement SQL 
					// parametrico di SELECT per le VALVOLE della SIMULAZIONE,
					// verifica se il parametro "n_iniziale" è una VALVOLA
	...
	TRY
	{
		...
		while (fwscanf(hydro2file, _T("%ld"), &nhandle) != EOF)
		{
			if (nhandle!=0)
			{
				...
				// Attraverso l'handle risalgo all'ads_name dell' entità
